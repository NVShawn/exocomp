...
#   --target-image IMAGE     Minimal base image (default: pinned Debian slim).
...
target_image="${CLEAN_TARGET_IMAGE:-}"
check_arch_mode=""
...
if [ -z "${target_image}" ]; then
  target_digest_var="CLEAN_TARGET_$(echo "${arch}" | tr '[:lower:]' '[:upper:]')_DIGEST"
...
if ! run_container_engine pull --platform "${target_platform}" "${target_image}"; then
  echo "failed to pull pinned clean target image for ${target_platform}: ${target_image}" >&2
  exit 1
fi

if ! run_container_engine pull --platform "${target_platform}" "${builder_image}"; then
  echo "failed to pull pinned builder used for clean target libraries: ${builder_image}" >&2
  exit 1
fi
...
qualified_target_image="localhost/exocomp-clean-target:${arch}"
if ! run_container_engine build \
    --platform "${target_platform}" \
    --pull=false \
    --file "${repo_root}/release/Containerfile.clean-target" \
    --tag "${qualified_target_image}" \
    --build-arg "BUILDER_IMAGE=${builder_image}" \
    --build-arg "TARGET_IMAGE=${target_image}" \
    --build-arg "LIB_ARCH=${lib_arch}" \
    "${repo_root}"; then
  echo "failed to assemble clean target image for ${target_platform}" >&2
  exit 1
fi
...
\"\${release_bin}\" eval \"
  smoke_ledger =
    Path.join(
      System.tmp_dir!(),
      \\\"exocomp-clean-container-\#{System.unique_integer([:positive])}.dets\\\"
    )
