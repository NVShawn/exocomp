...
cp -R "${repo_root}/test/fixtures/fake-release-valid" "${fixture_tmp}/valid"
cp -R "${repo_root}/test/fixtures/fake-release-undeclared" "${fixture_tmp}/undeclared"

valid_release="${fixture_tmp}/valid"
...
if [ -f "${report}" ]; then
  ...
  if [ "${first_char}" = "{" ] &&
    grep -q '"interpreter": "/lib64/ld-linux-x86-64.so.2"' "${report}"; then
    pass "dep-report.json includes parsed dependencies and interpreter"
  else
    fail_test "dep-report.json is missing JSON content or ELF interpreter"
  fi
else
...
tmp_no_erts="${fixture_tmp}/no-erts"
mkdir -p "${tmp_no_erts}"
...
# ---------------------------------------------------------------------------
# Test 9: readelf failures are rejected instead of producing a false pass
# ---------------------------------------------------------------------------
echo "Test 9: readelf failure — expect exit 2"
failing_readelf="${fixture_tmp}/failing-readelf"
printf '#!/bin/sh\nexit 1\n' > "${failing_readelf}"
chmod +x "${failing_readelf}"
if READELF="${failing_readelf}" \
  "${inspect}" amd64 "${valid_release}" "${baseline}" >/dev/null 2>&1; then
  fail_test "readelf failure should have failed inspection"
else
  exit_code=$?
  if [ "${exit_code}" -eq 2 ]; then
    pass "readelf failure correctly rejected with exit code 2"
  else
    fail_test "readelf failure returned ${exit_code} (expected 2)"
  fi
fi
