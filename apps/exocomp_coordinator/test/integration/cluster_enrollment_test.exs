...
defmodule Exocomp.Coordinator.ClusterEnrollmentTest do
  ...
  test "rejects a CSR with the wrong SPIFFE identity" do
    key = X509.PrivateKey.new_ec(:secp256r1)
    expected_cluster_id = "cluster-expected"
    csr = csr_pem(key, @organization_id, "cluster-other")

    assert {:error, %Error{code: :invalid_csr}} =
             ClusterIssuer.validate_csr(csr, @organization_id, expected_cluster_id)
  end
