# SPDX-FileCopyrightText: 2026 Exocomp contributors
# SPDX-License-Identifier: Apache-2.0
defmodule Exocomp.Coordinator.A2ARouterTest do
  use ExUnit.Case, async: true

  import Plug.Conn
  import Plug.Test

  alias Exocomp.A2A.{Artifact, DataPart, Message}
  alias Exocomp.Coordinator.A2ARouter
  alias Exocomp.Coordinator.TaskRegistry

  @base_opts [coordinator_id: "coordinator.example"]
  @concurrent_call_timeout_ms 10_000

  # ---------------------------------------------------------------------------
  # Helpers
  # ---------------------------------------------------------------------------

  defp start_registry(extra_opts \\ []) do
    name = :"coord_registry_#{System.unique_integer([:positive])}"
    start_supervised!({TaskRegistry, Keyword.put(extra_opts, :name, name)})
    name
  end

  defp build_opts(registry, extra \\ []) do
    opts =
      @base_opts
      |> Keyword.put(:registry, registry)
      |> Keyword.merge(extra)

    A2ARouter.init(opts)
  end

  defp authenticated_conn(method, path, body \\ nil) do
    method
    |> conn(path, body)
    |> put_peer_data(%{ssl_cert: <<1>>})
    |> put_req_header("a2a-version", "1.0")
  end

  defp json_conn(method, path, body) do
    method
    |> authenticated_conn(path, Jason.encode!(body))
    |> put_req_header("content-type", "application/json")
  end

  defp valid_message_body(skill_id \\ "exocomp.cluster.health", extra \\ %{}) do
    %{
      "role" => "user",
      "parts" => [
        %{"type" => "data", "data" => Map.merge(%{"skill" => skill_id}, extra)}
      ]
    }
  end

  defp registry_submit(registry, skill_id \\ "exocomp.cluster.health") do
    msg = %Message{
      role: :user,
      parts: [%DataPart{data: %{"skill" => skill_id}}]
    }

    {:ok, task_id} = TaskRegistry.submit(msg, skill_id, registry)
    task_id
  end

  # A dispatcher that immediately returns a success artifact.
  defmodule OkDispatcher do
    def dispatch(_skill_id, _params, _context \\ %{}) do
      {:ok,
       %Artifact{
         artifactId: "test-artifact-coord",
         name: "test",
         parts: [%DataPart{data: %{"nodes" => %{}}}]
       }}
    end
  end

  # A dispatcher that immediately returns an error.
  defmodule ErrDispatcher do
    def dispatch(_skill_id, _params, _context \\ %{}) do
      {:error, :simulated_cluster_failure}
    end
  end

  # An authorizer that rejects all selections.
  defmodule RejectAllAuthorizer do
    def authorize_selection(_params), do: {:error, :unauthorized}
  end

  # An authorizer that rejects specific node IDs.
  defmodule RestrictedAuthorizer do
    @allowed ["node-a", "node-b"]
    def authorize_selection(%{"node_ids" => ids}) when is_list(ids) do
      unauthorized = Enum.reject(ids, &(&1 in @allowed))
      if unauthorized == [], do: :ok, else: {:error, {:unauthorized_nodes, unauthorized}}
    end

    def authorize_selection(_params), do: :ok
  end

  # ---------------------------------------------------------------------------
  # Agent Card
  # ---------------------------------------------------------------------------

  test "agent card contains exactly the cluster skills with no remediation" do
    registry = start_registry()
    opts = build_opts(registry)

    response =
      :get
      |> authenticated_conn("/.well-known/agent-card.json")
      |> A2ARouter.call(opts)

    assert response.status == 200
    card = Jason.decode!(response.resp_body)

    assert card["url"] == "https://coordinator.example/"
    assert card["capabilities"]["streaming"] == false
    assert %{"id" => "ceph", "versions" => [1]} in card["clusterProfiles"]
    assert %{"id" => "default", "versions" => [1]} in card["clusterProfiles"]

    skill_ids = Enum.map(card["skills"], & &1["id"])
    assert "exocomp.cluster.health" in skill_ids
    assert "exocomp.cluster.diagnose" in skill_ids
    assert "exocomp.cluster.recover" in skill_ids

    # No arbitrary remediation execution skills should be present
    refute "exocomp.remediation.propose" in skill_ids
    refute "exocomp.remediation.execute" in skill_ids
  end

  test "agent card disables push notifications" do
    registry = start_registry()
    opts = build_opts(registry)

    response =
      :get
      |> authenticated_conn("/.well-known/agent-card.json")
      |> A2ARouter.call(opts)

    assert response.status == 200
    card = Jason.decode!(response.resp_body)
    assert card["capabilities"]["pushNotifications"] == false
  end

  # ---------------------------------------------------------------------------
  # mTLS authentication
  # ---------------------------------------------------------------------------

  test "request without a client certificate returns 401 before reading the body" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> conn("/message:send", String.duplicate("x", 1_048_577))
      |> put_req_header("a2a-version", "1.0")
      |> A2ARouter.call(opts)

    assert conn.status == 401
    assert %Plug.Conn.Unfetched{aspect: :body_params} = conn.body_params
  end

  test "unauthenticated request returns InvalidRequestError JSON" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> conn("/tasks")
      |> put_req_header("a2a-version", "1.0")
      |> A2ARouter.call(opts)

    assert conn.status == 401
    body = Jason.decode!(conn.resp_body)
    assert body["error"]["message"] =~ "certificate"
  end

  # ---------------------------------------------------------------------------
  # Version negotiation
  # ---------------------------------------------------------------------------

  test "missing A2A-Version header returns InvalidRequestError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> conn("/tasks")
      |> put_peer_data(%{ssl_cert: <<1>>})
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32600
  end

  test "wrong A2A-Version header value returns InvalidRequestError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> conn("/tasks")
      |> put_peer_data(%{ssl_cert: <<1>>})
      |> put_req_header("a2a-version", "2.0")
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32600
  end

  # ---------------------------------------------------------------------------
  # Body size limit
  # ---------------------------------------------------------------------------

  test "body over one MiB returns 413" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> authenticated_conn("/message:send", String.duplicate("x", 1_048_577))
      |> put_req_header("content-type", "application/json")
      |> A2ARouter.call(opts)

    assert conn.status == 413
  end

  # ---------------------------------------------------------------------------
  # Unsupported capabilities
  # ---------------------------------------------------------------------------

  test "message streaming returns UnsupportedOperationError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> authenticated_conn("/message/stream", Jason.encode!(%{}))
      |> put_req_header("content-type", "application/json")
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32004
  end

  test "task resubscribe returns UnsupportedOperationError" do
    registry = start_registry()
    opts = build_opts(registry)
    task_id = registry_submit(registry)

    conn =
      :post
      |> authenticated_conn("/tasks/#{task_id}/resubscribe", Jason.encode!(%{}))
      |> put_req_header("content-type", "application/json")
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32004
  end

  test "unknown route returns MethodNotFoundError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> authenticated_conn("/not-a-route")
      |> A2ARouter.call(opts)

    assert conn.status == 404
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32601
  end

  # ---------------------------------------------------------------------------
  # No remediation execution skill
  # ---------------------------------------------------------------------------

  test "remediation skill returns InvalidParamsError (no execution path)" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.remediation.propose"))
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32602
  end

  test "node-level skill returns InvalidParamsError (wrong skill domain)" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.system.diagnose"))
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32602
  end

  # ---------------------------------------------------------------------------
  # POST /message:send — valid cluster skills
  # ---------------------------------------------------------------------------

  test "POST /message:send: cluster.health returns 202 with task in submitted state" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: OkDispatcher)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.cluster.health"))
      |> A2ARouter.call(opts)

    assert conn.status == 202
    body = Jason.decode!(conn.resp_body)
    assert is_binary(body["id"])
    assert body["status"]["state"] == "submitted"

    assert_task_state(body["id"], registry, :completed, 500)
  end

  test "POST /message:send: cluster.diagnose returns 202 with task in submitted state" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: OkDispatcher)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.cluster.diagnose"))
      |> A2ARouter.call(opts)

    assert conn.status == 202
    body = Jason.decode!(conn.resp_body)
    assert is_binary(body["id"])
    assert body["status"]["state"] == "submitted"

    assert_task_state(body["id"], registry, :completed, 500)
  end

  test "POST /message:send: invalid JSON body returns 400 InvalidRequestError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> authenticated_conn("/message:send", "not-valid-json{{{")
      |> put_req_header("content-type", "application/json")
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32600
  end

  test "POST /message:send: missing parts returns 400 InvalidParamsError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> json_conn("/message:send", %{"role" => "user"})
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32602
  end

  test "POST /message:send: unknown skill returns 400 InvalidParamsError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.unknown.skill"))
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32602
  end

  test "POST /message:send: at_capacity returns 429" do
    registry = start_registry(max_concurrent_tasks: 1)
    opts = build_opts(registry)

    task_id = registry_submit(registry)
    :ok = TaskRegistry.transition(task_id, :working, nil, registry)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body())
      |> A2ARouter.call(opts)

    assert conn.status == 429
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32603
  end

  # ---------------------------------------------------------------------------
  # Selection authorization
  # ---------------------------------------------------------------------------

  test "unauthorized node selection is rejected with 400 InvalidParamsError" do
    registry = start_registry()
    opts = build_opts(registry, authorizer: RejectAllAuthorizer)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.cluster.health"))
      |> A2ARouter.call(opts)

    assert conn.status == 400
    body = Jason.decode!(conn.resp_body)
    assert body["error"]["code"] == -32602
    assert body["error"]["message"] =~ "Unauthorized"
  end

  test "selection with valid node IDs is accepted by restricted authorizer" do
    registry = start_registry()
    opts = build_opts(registry, authorizer: RestrictedAuthorizer, dispatcher: OkDispatcher)

    conn =
      :post
      |> json_conn(
        "/message:send",
        valid_message_body("exocomp.cluster.health", %{"node_ids" => ["node-a"]})
      )
      |> A2ARouter.call(opts)

    assert conn.status == 202
    assert_task_state(Jason.decode!(conn.resp_body)["id"], registry, :completed, 500)
  end

  test "selection with unauthorized node IDs returns error listing the unauthorized nodes" do
    registry = start_registry()
    opts = build_opts(registry, authorizer: RestrictedAuthorizer)

    conn =
      :post
      |> json_conn(
        "/message:send",
        valid_message_body("exocomp.cluster.health", %{"node_ids" => ["node-c", "node-d"]})
      )
      |> A2ARouter.call(opts)

    assert conn.status == 400
    body = Jason.decode!(conn.resp_body)
    assert body["error"]["code"] == -32602
    # Error message should mention the unauthorized node IDs
    assert body["error"]["message"] =~ "node-c"
    assert body["error"]["message"] =~ "node-d"
  end

  test "message with no node_ids selection passes default authorizer" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: OkDispatcher)

    conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.cluster.diagnose"))
      |> A2ARouter.call(opts)

    assert conn.status == 202
    assert_task_state(Jason.decode!(conn.resp_body)["id"], registry, :completed, 500)
  end

  # ---------------------------------------------------------------------------
  # GET /tasks/:id
  # ---------------------------------------------------------------------------

  test "GET /tasks/:id: known task returns 200 with task JSON" do
    registry = start_registry()
    opts = build_opts(registry)

    task_id = registry_submit(registry)

    conn =
      :get
      |> authenticated_conn("/tasks/#{task_id}")
      |> A2ARouter.call(opts)

    assert conn.status == 200
    body = Jason.decode!(conn.resp_body)
    assert body["id"] == task_id
    assert body["status"]["state"] == "submitted"
  end

  test "GET /tasks/:id: unknown id returns 404 TaskNotFoundError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> authenticated_conn("/tasks/nonexistent-task-id")
      |> A2ARouter.call(opts)

    assert conn.status == 404
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32001
  end

  # ---------------------------------------------------------------------------
  # GET /tasks
  # ---------------------------------------------------------------------------

  test "GET /tasks: returns list including the submitted task" do
    registry = start_registry()
    opts = build_opts(registry)

    task_id = registry_submit(registry)

    conn =
      :get
      |> authenticated_conn("/tasks")
      |> A2ARouter.call(opts)

    assert conn.status == 200
    tasks = Jason.decode!(conn.resp_body)
    assert is_list(tasks)
    assert task_id in Enum.map(tasks, & &1["id"])
  end

  test "GET /tasks: returns empty list when no tasks exist" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :get
      |> authenticated_conn("/tasks")
      |> A2ARouter.call(opts)

    assert conn.status == 200
    assert Jason.decode!(conn.resp_body) == []
  end

  # ---------------------------------------------------------------------------
  # POST /tasks/:id:cancel
  # ---------------------------------------------------------------------------

  test "POST /tasks/:id:cancel: submitted task returns 200 with canceled task" do
    registry = start_registry()
    opts = build_opts(registry)

    task_id = registry_submit(registry)

    conn =
      :post
      |> authenticated_conn("/tasks/#{task_id}:cancel")
      |> A2ARouter.call(opts)

    assert conn.status == 200
    body = Jason.decode!(conn.resp_body)
    assert body["id"] == task_id
    assert body["status"]["state"] == "canceled"
  end

  test "POST /tasks/:id:cancel: completed task returns 400 TaskNotCancelableError" do
    registry = start_registry()
    opts = build_opts(registry)

    task_id = registry_submit(registry)
    :ok = TaskRegistry.transition(task_id, :working, nil, registry)
    :ok = TaskRegistry.transition(task_id, :completed, nil, registry)

    conn =
      :post
      |> authenticated_conn("/tasks/#{task_id}:cancel")
      |> A2ARouter.call(opts)

    assert conn.status == 400
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32002
  end

  test "POST /tasks/:id:cancel: unknown id returns 404 TaskNotFoundError" do
    registry = start_registry()
    opts = build_opts(registry)

    conn =
      :post
      |> authenticated_conn("/tasks/nonexistent-task:cancel")
      |> A2ARouter.call(opts)

    assert conn.status == 404
    assert Jason.decode!(conn.resp_body)["error"]["code"] == -32001
  end

  # ---------------------------------------------------------------------------
  # Async skill execution lifecycle
  # ---------------------------------------------------------------------------

  test "async skill execution: success transitions task to :completed with artifact" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: OkDispatcher)

    send_conn =
      :post
      |> json_conn("/message:send", valid_message_body())
      |> A2ARouter.call(opts)

    assert send_conn.status == 202
    task_id = Jason.decode!(send_conn.resp_body)["id"]

    assert_task_state(task_id, registry, :completed, 500)

    {:ok, task} = TaskRegistry.get(task_id, registry)
    assert task.status.state == :completed
    assert %Artifact{} = task.status.message
  end

  test "async skill execution: skill error transitions task to :failed" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: ErrDispatcher)

    send_conn =
      :post
      |> json_conn("/message:send", valid_message_body())
      |> A2ARouter.call(opts)

    assert send_conn.status == 202
    task_id = Jason.decode!(send_conn.resp_body)["id"]

    assert_task_state(task_id, registry, :failed, 500)

    {:ok, task} = TaskRegistry.get(task_id, registry)
    assert task.status.state == :failed
    assert task.status.message == :simulated_cluster_failure
  end

  # ---------------------------------------------------------------------------
  # Concurrent callers
  # ---------------------------------------------------------------------------

  test "concurrent callers each receive their own task ID" do
    registry = start_registry(max_concurrent_tasks: 5)
    opts = build_opts(registry, dispatcher: OkDispatcher)

    tasks =
      1..5
      |> Enum.map(fn _ ->
        Task.async(fn ->
          :post
          |> json_conn("/message:send", valid_message_body())
          |> A2ARouter.call(opts)
        end)
      end)
      |> Task.await_many(@concurrent_call_timeout_ms)

    task_ids = Enum.map(tasks, fn conn -> Jason.decode!(conn.resp_body)["id"] end)
    statuses = Enum.map(tasks, & &1.status)

    # All should be accepted
    assert Enum.all?(statuses, &(&1 == 202))
    # All task IDs should be unique
    assert length(Enum.uniq(task_ids)) == 5
  end

  # ---------------------------------------------------------------------------
  # Partial results
  # ---------------------------------------------------------------------------

  test "cluster skill returns correlated node results in artifact" do
    registry = start_registry()
    opts = build_opts(registry, dispatcher: OkDispatcher)

    send_conn =
      :post
      |> json_conn("/message:send", valid_message_body("exocomp.cluster.health"))
      |> A2ARouter.call(opts)

    task_id = Jason.decode!(send_conn.resp_body)["id"]
    assert_task_state(task_id, registry, :completed, 500)

    get_conn =
      :get
      |> authenticated_conn("/tasks/#{task_id}")
      |> A2ARouter.call(opts)

    assert get_conn.status == 200
    body = Jason.decode!(get_conn.resp_body)
    assert body["status"]["state"] == "completed"
    # The artifact should be present
    assert is_list(body["artifacts"])
    assert length(body["artifacts"]) == 1
    [artifact] = body["artifacts"]
    assert artifact["name"] == "test"
  end

  # ---------------------------------------------------------------------------
  # Helpers for async tests
  # ---------------------------------------------------------------------------

  defp assert_task_state(task_id, registry, expected_state, timeout_ms) do
    deadline = System.monotonic_time(:millisecond) + timeout_ms

    Enum.reduce_while(Stream.repeatedly(fn -> :poll end), :waiting, fn _tick, _acc ->
      case TaskRegistry.get(task_id, registry) do
        {:ok, %{status: %{state: ^expected_state}}} ->
          {:halt, :done}

        {:ok, _other} ->
          if System.monotonic_time(:millisecond) >= deadline do
            {:halt, :timeout}
          else
            Process.sleep(5)
            {:cont, :waiting}
          end

        {:error, reason} ->
          {:halt, {:error, reason}}
      end
    end)
    |> case do
      :done ->
        :ok

      :timeout ->
        {:ok, task} = TaskRegistry.get(task_id, registry)

        flunk(
          "Task #{task_id} did not reach state #{expected_state} " <>
            "within #{timeout_ms}ms; current: #{task.status.state}"
        )

      {:error, reason} ->
        flunk("TaskRegistry.get failed: #{inspect(reason)}")
    end
  end
end
