...
defmodule Exocomp.Coordinator.A2ARouter do
  @moduledoc """
  HTTP entry point for the coordinator's A2A 1.0 cluster service.
  ...
  ## Accepted router options

  Passed through `init/1` and accessible via `conn.assigns.router_opts`:

  - `:coordinator_id` — coordinator identifier forwarded to the Agent Card handler.
  - `:registry`   — TaskRegistry GenServer name (default
    `Exocomp.Coordinator.TaskRegistry`).
  - `:dispatcher` — Dispatcher module (default
    `Exocomp.Coordinator.Skills.Dispatcher`).
  - `:authorizer` — Module implementing `authorize_selection/1` (default
    `Exocomp.Coordinator.InventoryAuthorizer`). Used to validate that
    requested node IDs exist in the inventory.
  ...
  Remediation execution skills are intentionally unreachable. Streaming and
  push notifications are declared unsupported.
  """
  ...
  alias Exocomp.Coordinator.Handlers.AgentCardHandler
  alias Exocomp.Coordinator.InventoryAuthorizer
  alias Exocomp.Coordinator.Plug.JSONBodyParser
  alias Exocomp.Coordinator.Skills.Dispatcher
  ...
  post "/message:send" do
    ...
    with {:ok, message} <- Codec.decode_message(conn.body_params),
      ...
      {:error, {:unauthorized_nodes, unauthorized}} ->
        error_response(conn, 400, %InvalidParamsError{
          message: "Unauthorized node selection: #{Enum.join(unauthorized, ", ")}"
        })

      {:error, :unauthorized} ->
        error_response(conn, 400, %InvalidParamsError{
          message: "Unauthorized node selection"
        })
    ...
    Task.start(fn ->
      case TaskRegistry.transition(task_id, :working, nil, registry) do
        :ok ->
          ...
          timeout_ms =
            Application.get_env(:exocomp_coordinator, :skill_timeout_ms, 30_000)
  ...
  defp authenticate_mtls(conn, _opts) do
    case Plug.Conn.get_peer_data(conn) do
      %{ssl_cert: ssl_cert} when is_binary(ssl_cert) and byte_size(ssl_cert) > 0 ->
        Logger.info("[CoordinatorA2ARouter] mTLS client certificate accepted")
        conn

      _peer_data ->
        Logger.warning("[CoordinatorA2ARouter] mTLS client certificate missing")
  ...
  defp authorizer_opt(conn) do
    Keyword.get(conn.assigns.router_opts || [], :authorizer, InventoryAuthorizer)
  end
