...
defmodule Exocomp.MissionControl.OIDCIntegrationTest do
  ...
  test "completes a signed OIDC login with PKCE and stores only identity claims" do
    ...
    assert MapSet.member?(cookie_attributes, "secure")
    assert MapSet.member?(cookie_attributes, "httponly")
    assert MapSet.member?(cookie_attributes, "samesite=lax")
    assert MapSet.member?(cookie_attributes, "max-age=604800")
