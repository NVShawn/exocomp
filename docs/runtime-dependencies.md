...
| `libtinfo.so.6` | Terminal capability support used by ERTS |
| `libcrypto.so.3` | OpenSSL cryptography used by the OTP crypto application |
...
directory. The script uses host `readelf`; cross-architecture inspection
(arm64 ELF on an amd64 host) works because metadata parsing does not execute
the target binary.
...
2. Run host `readelf -d -l`. GNU binutils `readelf` is
   architecture-independent, so the host tool can inspect both amd64 and arm64
   release artifacts without executing them.
...
The inspector uses `readelf` from `PATH` by default. Set `READELF` to select a
...
automatically when it inspects each release after the containerized build.

### Cross-architecture inspection on a development host

Cross-architecture inspection does not require binfmt/QEMU because `readelf`
parses ELF metadata without executing the target binary. Cross-architecture
release builds and smoke tests still require the container engine capability
documented in the development and qualification guides.
